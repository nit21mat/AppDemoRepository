				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("SoaIdRes") === oObject.SoaIdRes;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].SoaIdRes;
										} else {
											sCurrentId = aItemsNotInTheList[0].SoaIdRes;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("SOASearchResultSet", {
											SoaIdRes : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("SoaIdRes"),
							title: oBindingContext.getProperty("SoaIdRes")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "SOASearchResultSet (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'SOASearchResultSet (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("SoaIdRes");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all SOASearchResultSet to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "SOASearchResultSet (" + iExpectedLength + ")"}),
									success : function () {
